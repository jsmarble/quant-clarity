...
Implementation follows [`docs/design/implementation-plan.md`](docs/design/implementation-plan.md): machine contracts and canonical rules first, then one lawful structured-provider slice through atomic publication, API, and web delivery. [Phase 5S](docs/design/phase-5s-signed-frontend-metadata.md) now carries canonical publication metadata through the actual web/API/query Worker chain with signed, identity-free internal requests. [Phase 5T](docs/design/phase-5t-local-methodology-acceptance.md) closes the local human-readable methodology prerequisite with semantic version/effective-date markup, a stable historical page, a material-change log, and browser acceptance for `FE-050`–`FE-052`. [Phase 5U](docs/design/phase-5u-publication-pinned-model-facts.md) defines the next local-only signed, publication-pinned canonical Model Facts slice while keeping the public Model API route closed. These are local prerequisites only: remote configuration, production data, deployment, legal/accountability approval, and release acceptance remain pending. ADR 0012 resolves the former Pages conflict: the web runtime uses current Astro SSR on Cloudflare Workers with Static Assets while preserving the separate API/query boundaries.
