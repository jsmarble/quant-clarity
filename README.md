...
Implementation follows [`docs/design/implementation-plan.md`](docs/design/implementation-plan.md): machine contracts and canonical rules first, then one lawful structured-provider slice through atomic publication, API, and web delivery. [Phases 5S through 5X](docs/design/implementation-plan.md) carry publication metadata, methodology, canonical Model Facts, exact discovery, compact Model cards, and explicit-Variant cards through signed, identity-free local web/API/query reads. Model and Variant cards project from canonical resources the exact tiers already read, with zero extra SQL and no provider-derived content beyond the approved count. These are local prerequisites only: Variant detail, provider comparison, pipeline Workflow assembly, remote configuration, production data, deployment, legal/accountability approval, and release acceptance remain pending. ADR 0012 resolves the former Pages conflict: the web runtime uses current Astro SSR on Cloudflare Workers with Static Assets while preserving the separate API/query boundaries.
