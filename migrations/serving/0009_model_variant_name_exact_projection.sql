-- Canonical model/variant exact-name BLOB projection and serving proof v3 cutover.
-- Requirements: SRCH-002, SRCH-006-SRCH-010, PIPE-033, PIPE-039A,
-- PIPE-044, PIPE-050-PIPE-053, BE-003, BE-011, CF-022, SEC-005,
-- SEC-007, QA-001, QA-005, QA-006.

PRAGMA defer_foreign_keys = true;

-- Advance only the exact schema produced by migration 0008. V3 evidence
-- cannot be fabricated for an already sealed, readied, or switched v2
-- publication, so proof-bearing state is rejected before any schema mutation.
SELECT CASE WHEN (
  SELECT count(*) FROM serving_schema_metadata
) <> 1 OR (
  SELECT count(*) FROM serving_schema_metadata
  WHERE singleton = 1 AND schema_version = '1.5.1'
) <> 1 THEN json('') END;
...
-- Fail before mutation if a prior partial/manual attempt left any v3 object.
SELECT CASE WHEN EXISTS (
...
CREATE TABLE publication_model_variant_name_search_document (
...
-- Replace the provider-only seal boundary with a combined provider and
-- model/variant projection boundary. The provider NUL guard from 0008 remains.
DROP TRIGGER publication_provider_search_seal_guard;
...
END;
-- All v2 proof rows were rejected above. Replace their closed schemas without
-- inventing v3 evidence.
...
CREATE TABLE publication_readiness_receipt (
  ...
  receipt_version TEXT NOT NULL CHECK (receipt_version = '3.0.0'),
...
CREATE TABLE publication_probe_receipt (
  ...
  probe_set_version TEXT NOT NULL CHECK (probe_set_version = 'search-gold@3'),
...
CREATE TABLE publication_readiness_attestation (
  ...
  closure_hash TEXT NOT NULL CHECK (length(closure_hash) = 71 AND substr(closure_hash, 1, 7) = 'sha256:' AND substr(closure_hash, 8) NOT GLOB '*[^0-9a-f]*'),
  bundle_hash TEXT NOT NULL CHECK (length(bundle_hash) = 71 AND substr(bundle_hash, 1, 7) = 'sha256:' AND substr(bundle_hash, 8) NOT GLOB '*[^0-9a-f]*'),
  evaluator_version TEXT NOT NULL CHECK (evaluator_version = '3.0.0'),
...
BEGIN
  ...
  SELECT CASE WHEN NOT EXISTS (
    SELECT 1 FROM publication_probe_receipt
    WHERE publication_id = NEW.publication_id
      AND probe_set_version = 'search-gold@3'
...
CREATE TABLE publication_switch_preflight (
  ...
  preflight_version TEXT NOT NULL CHECK (preflight_version = '3.0.0'),
