...
B2C-B must define `backup-v2@1` with format version `2.0.0`, a new protected backup/recovery catalog identity, `backup-v2-restore-source@1`, and `serving-restore-rebuild@6`. The complete recovery authority must independently bind the base publication closure/bundle and exact sidecar artifact digest. Export may inventory ordinary proof/mapping rows for migration-away completeness, but restore never imports them as truth: it rebuilds them from the independently verified base bundle plus read-only verified sidecar in a fresh isolated schema `1.13.0`, then rebuilds FTS/Vectorize/projections, readiness, and optional local head state. Cloudflare currently documents that D1 export does not support virtual tables, including FTS5, and that a running export blocks other database requests. Those constraints reinforce the existing writer-drained, ordinary-source, rebuild-index design.
