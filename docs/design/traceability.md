...
[ADR 0026](../decisions/0026-blob-model-variant-exact-search-cutover.md) defines the later atomic A2 schema/readiness/switch/restore boundary for `PIPE-044` and `PIPE-050`–`PIPE-056`. Implemented [Phase 5G-A1](phase-5g-a1-model-variant-durable-proof-core.md) contributes only local structural rows, nominal revision-bound staging, and exact storage-parity proof primitives; it does not satisfy the D1, activation, rollback, restore, public-version, or deployed evidence, so all affected pipeline rows remain `Planned`.
...
[ADR 0026](../decisions/0026-blob-model-variant-exact-search-cutover.md) and implemented [Phase 5G-A1](phase-5g-a1-model-variant-durable-proof-core.md) contribute local controlled-writer and reconstructible-search design/evidence to `BE-003` and `BE-010`–`BE-012`. Durable D1 writing, portable restore, migration-away export, reader rehydration, and deployment remain pending, so no backend row advances.
