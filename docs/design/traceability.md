...
[Phase 5A](phase-5a-local-api-kernel.md) defines local targets contributing to `QA-004`–`QA-006` and `QA-014`; [Phase 5D](phase-5d-local-query-rpc.md) adds a narrow local/runtime RPC and D1 Session verification boundary; [Phase 5E](phase-5e-search-collection-degradation.md) adds structural, semantic-equality, unknown-tolerance, and empty-example contract checks. Complete API, search, publication, cache, abuse, and multi-PoP artifacts are still pending.
