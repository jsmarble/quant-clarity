...
| `SRCH-002` | §10 | Exact canonical model names, exact provider model IDs, and exact provider names shall appear before… | `D10`, `D16`, `D19` | Search acceptance — `SAT-SRCH-002`; [Phase 4D1 FTS fields and provider-name gap](phase-4d1-readiness-ledger.md); [ADR 0021 provider-name decision](../decisions/0021-canonical-provider-exact-search.md); [Phase 4E trusted provider projection](phase-4e-provider-search-core.md); [Phase 4G durable provider index](phase-4g-provider-search-schema-writers.md); [Phase 5B internal provider exact reader](phase-5b-provider-exact-name-reader.md); [Phase 5C NUL boundary](phase-5c-provider-name-nul-boundary.md); [Phase 5D local RPC provider tier](phase-5d-local-query-rpc.md), other tiers/public query pending | Planned |
