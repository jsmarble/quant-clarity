...
[Phase 5A](phase-5a-local-api-kernel.md) records bounded local decision targets for `SRCH-002`, `SRCH-004`–`SRCH-006`, `SRCH-008`, and `SRCH-009`. [Phase 5B](phase-5b-provider-exact-name-reader.md) adds a selected-publication lifecycle recheck and canonical-rehydrating provider exact-name D1 reader. [ADR 0022](../decisions/0022-forbid-nul-provider-display-names.md) and [Phase 5C](phase-5c-provider-name-nul-boundary.md) resolve its U+0000 contract/storage design mismatch without changing normalization version 1. [ADR 0023](../decisions/0023-local-query-rpc-and-bookmark-continuity.md) and [Phase 5D](phase-5d-local-query-rpc.md) define the first local two-method RPC and bookmark-continuous provider tier while preserving the incomplete-tier, cursor, and empty-degradation blockers. These slices deliberately omit public search integration and leave every search row `Planned` pending the complete acceptance set and deployed search evidence.

| Source ID | PRD | Requirement summary | Planned design sections | Planned primary verification | Status |
|---|---|---|---|---|---|
| `SRCH-001` | §10 | Search shall combine exact/keyword retrieval with semantic vector retrieval. | `D10`, `D16`, `D19` | Search acceptance — `SAT-SRCH-001`; [Phase 4D1 local FTS](phase-4d1-readiness-ledger.md), semantic runtime pending | Planned |
...
| `SRCH-010` | §10 | Search quality shall be evaluated against version-controlled exact, alias, filter, semantic, and no-result… | `D10`, `D16`, `D19` | Search acceptance — `SAT-SRCH-010`; [ADR 0021 provider-name gold cases](../decisions/0021-canonical-provider-exact-search.md) | Planned |
...
| `API-010` | §11.1 | The API shall expose search with the same exact-first behavior and structured filters as the web interface. | `D09`, `D16`, `D19` | API conformance — `ACT-API-010`; [Phase 5D local provider tier and incomplete-search blocker](phase-5d-local-query-rpc.md) | Planned |
...
| `NFR-006` | §15 | Failure of semantic search shall degrade to exact/structured discovery where feasible rather than make model… | `D14`, `D15`, `D16`, `D19` | Performance/reliability — `PRT-NFR-006`; [Phase 5D empty-collection degradation blocker](phase-5d-local-query-rpc.md) | Planned |
...
[Phase 5A](phase-5a-local-api-kernel.md) defines local targets contributing to `QA-004`–`QA-006` and `QA-014`; [Phase 5D](phase-5d-local-query-rpc.md) adds a narrow local/runtime RPC and D1 Session verification boundary. Complete API, search, publication, cache, abuse, and multi-PoP artifacts are still pending.
...
| `QA-004` | §21 | API conformance tests shall validate OpenAPI examples, pagination, filters, sorting, caching, CORS, nulls,… | `D16`, `D19` | Quality-gate audit — `QGA-QA-004`; [Phase 5D local RPC adapter boundary](phase-5d-local-query-rpc.md), public conformance pending | Planned |
| `QA-005` | §21 | Search tests shall cover exact IDs, aliases, provider names, precision phrases, natural-language intent,… | `D16`, `D19` | Quality-gate audit — `QGA-QA-005`; [ADR 0021 provider-name validation matrix](../decisions/0021-canonical-provider-exact-search.md); [Phase 5B focused exact-provider tests](phase-5b-provider-exact-name-reader.md); [Phase 5C U+0000 boundary matrix](phase-5c-provider-name-nul-boundary.md); [Phase 5D bookmark-continuous provider tier](phase-5d-local-query-rpc.md), full search set pending | Planned |
