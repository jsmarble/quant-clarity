...
Phase 5G is split by [ADR 0026](../decisions/0026-blob-model-variant-exact-search-cutover.md) so the publication-format cutover cannot be confused with query integration. [Phase 5G-A1](phase-5g-a1-model-variant-durable-proof-core.md) implements the runtime-neutral frozen UTF-8 byte rows, closure/revision-bound staging projection, and six-field storage-artifact proof only. Phase 5G-A2 must later land the `STRICT` BLOB table/index, bounded lowercase-hex/`unhex` D1 writer, schema `1.6.0`, seven-field v3 readiness/switch family, seal completeness, and restore rebuild as one atomic boundary. Phase 5G-B then adds the bounded canonical-rehydrating equality reader and bookmark-continuous RPC seam without creating a public route. No migration or deployment is authorized by A1, every mapped traceability row remains `Planned`, and A2/B acceptance remains pending.
