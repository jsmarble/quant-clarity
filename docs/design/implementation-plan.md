...
Exit evidence: local decision evidence is recorded in [`phase-4-local-kernel.md`](phase-4-local-kernel.md), [`phase-4b-serving-dispositions.md`](phase-4b-serving-dispositions.md), [`phase-4c-sealed-closure.md`](phase-4c-sealed-closure.md), [`phase-4d1-readiness-ledger.md`](phase-4d1-readiness-ledger.md), and [`phase-4d2-exact-generation-switching.md`](phase-4d2-exact-generation-switching.md). The fixed deployed D1 batch and all remote artifact, reader, backup, and chaos evidence remain pending. `GATE-publication-chaos`, `GATE-search-acceptance`, and `GATE-restore-and-rebuild` still must pass for the deployed vertical slice before Phase 4 exits.
