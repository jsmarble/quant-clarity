...
Phase 5H is split by [ADR 0027](../decisions/0027-trusted-provider-model-id-projection.md) and [ADR 0028](../decisions/0028-provider-model-id-durable-storage-cutover.md). [Phase 5H-A1](phase-5h-a1-provider-model-id-projection.md) and [Phase 5H-A2](phase-5h-a2-provider-model-id-storage.md) are locally implemented: the complete nominal projection now has schema `1.7.0` dual-BLOB storage, bounded atomic staging, forced raw and normalized index proof, v4 readiness and switch evidence, legacy-version rejection, backup exclusion, and local restore-rebuild integration. No remote resource, deployment, operational restore, or release evidence is claimed. Phase 5H-B still requires reviewed reader and composition decisions for status eligibility, raw-versus-normalized public matching, query reachability, result semantics, deduplication/order, and the merged cursor. All traceability statuses remain unchanged.
