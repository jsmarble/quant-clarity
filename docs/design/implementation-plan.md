...
Status: in progress. A runtime-neutral publication kernel now provides locally testable immutable-closure hashing, provider-slice/search/vector inventory, readiness, activation/rollback, switch-history, backup-manifest, and no-pruning decisions under [ADR 0015](../decisions/0015-publication-closure-and-lifecycle.md). [Phase 4B](phase-4b-serving-dispositions.md) adds local serving-D1 persistence for null unavailable dispositions, stable carried-forward lineage, and fail-closed non-empty readiness under [ADR 0017](../decisions/0017-provider-disposition-persistence.md). [Phase 4C](phase-4c-sealed-closure.md) and [ADR 0018](../decisions/0018-sealed-serving-closure-persistence.md) add local atomic migration and exact row-projection evidence for every reproducible closure input, a monotone staging revision, canonical content/chunk hashing, and a closed readiness/head boundary. Phase 4D readiness receipts/FTS/transactional switch history, deployed D1 execution, Cloudflare Vectorize visibility, populated-cache chaos, writer-drained backup, isolated restore, physical pruning, preview resources, and all composite gates remain pending.
...
Exit evidence: local decision evidence is recorded in [`phase-4-local-kernel.md`](phase-4-local-kernel.md), [`phase-4b-serving-dispositions.md`](phase-4b-serving-dispositions.md), and [`phase-4c-sealed-closure.md`](phase-4c-sealed-closure.md). Phase 4D must follow Phase 4C by persisting closure-bound readiness receipts before transactional switch history can authorize a head change. `GATE-publication-chaos`, `GATE-search-acceptance`, and `GATE-restore-and-rebuild` still must pass for the deployed vertical slice before Phase 4 exits.
