...
Phase 5H is split by [ADR 0027](../decisions/0027-trusted-provider-model-id-projection.md) before the exact provider-model-ID tier can be made durable or queryable. [Phase 5H-A1](phase-5h-a1-provider-model-id-projection.md) is locally implemented and accepts only a runtime-neutral, complete, nominal projection: one deterministic Offering-ID-ordered row per manifest Offering, exact raw and pinned-normalized provider model IDs, enabled-provider attribution, exact Model/Variant target binding, and both canonical resource hashes. It adds no FTS/schema/migration/writer/readiness/switch/restore/reader/RPC/composition/cursor/route surface. A future A2 requires a reviewed durable cutover decision; a future B requires reviewed reader and composition decisions for status eligibility, raw-versus-normalized public matching, query reachability, result semantics, deduplication/order, and the merged cursor. All traceability statuses remain unchanged.
