...
The next bounded slice, H2b, must independently encode and hash this graph in Node and workerd, apply the typed scope/order plan, reject order-key collisions and unconsumed rows, recompute child counts/roots before parent leaves, and prove the successor-to-authority-to-refused-receipt cascade. Even after H2b, document/anchor resolvers, the complete semantic oracle, reviewed build manifest, migration 0010 and frozen D1 enumeration, accepted aggregate limits and all protected writers remain pending.
