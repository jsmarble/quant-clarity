...
The accepted scheduled-Workflow architecture is not implemented by these frontend slices. Current predeployment policy forbids Workflow/data bindings in local Wrangler configuration, and the inert preview plan leaves the reserved `PublicationWorkflow` unprovisioned and unscheduled. A separately authorized platform increment must establish those protected bindings, non-visitor control-plane schema, and runtime evidence before preview deployment.
