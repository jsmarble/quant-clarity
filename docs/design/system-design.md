...
Exact retrieval normalizes Unicode case, punctuation, separators, and organization prefixes without merging explicit variants. Publication-scoped D1 alias tables and FTS5 provide exact, prefix, keyword, provider-name, and provider-model-ID candidates. Exact identity matches precede prefix/keyword matches; deterministic model name and stable ID break model-search ties without provider influence.
