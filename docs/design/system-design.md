...
The accepted scheduled-Workflow orchestration remains unimplemented by these frontend slices. ADR 0058 and Phase 7.2-A now provide only an actual dormant class and pure scheduled-occurrence planner. Current predeployment policy still forbids Workflow/data bindings in local Wrangler configuration, and the inert preview plan leaves `PublicationWorkflow` unprovisioned and unscheduled. A separately authorized platform increment must establish protected bindings, exact run-plan/provider authority, non-visitor control-plane persistence, canonical status semantics, native Workflow runtime evidence, and environment isolation before preview deployment.
