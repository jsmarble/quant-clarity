...
[ADR 0042](../decisions/0042-model-slug-lifecycle-authority.md) and locally implemented [Phase 5O-B2C-A](phase-5o-b2c-a-model-slug-lifecycle.md) define the schema-`1.13.0` lifecycle authority. Migration 0016 accepts only pristine dormant `1.12.0`, exact-inventories every retained logical/FTS object plus the pinned Wrangler/workerd control-table forms, adds an exact partial unique current-Model index, makes the staged artifact proof and complete current/historical parity closure-seal prerequisites, and replaces v4 readiness/switch state with version `5.0.0` receipts, attestation, and preflight plus `search-gold@5`. Readiness binds the sidecar digest, source-history root, mapping root/counts, exact serving parity, and both indexes. Activation requires fresh unexpired v5 readiness; rollback instead rereads and hostile-verifies the exact retained sidecar against protected retained-base authority before an exact-generation switch. Backup-v1 rejects `1.13.0`; B2C-B owns a two-artifact backup-v2 and isolated rebuild, B2C-C owns the internal publication-pinned lookup, and B3 owns all public slug behavior. No visitor data, public mutation, remote resource, or deployment is introduced.

[ADR 0043](../decisions/0043-byte-authentic-publication-recovery.md) and [Phase 5O-B2C-B](phase-5o-b2c-b-portable-recovery.md) close a prerequisite that descriptor-only backup-v1 never supplied: independently retrievable and verifiable base publication bytes. B2C-B1 defines a closed content-addressed `publication-recovery-base@1` manifest/chunk archive and exact-key hostile reader that replays persisted-content, inventory, immutable-manifest, and closure semantics before returning byte-and-semantic verification. Its pinned-workerd joint row/byte profile repeats an identical 18-object archive and full replay for 50,000 public-contract-valid rows and 25,148,376 bytes including the root. The production source-byte admission estimate is 84,267,088 bytes under the 96 MiB budget; the stricter including-root test estimate is 84,285,360 bytes. The 1,024/1,025 read-body chunk boundary is covered. A separate independent canonical packer proves the verifier's object ceiling through pinned-workerd replay of exactly 63 source objects plus root and unit rejection of 64 source objects plus root before a source read. B2C-B2 will bind that verified result and the existing Model-slug sidecar through `backup-v2@1`, an independently protected expected catalog digest, `backup-v2-restore-source@1`, and a fresh schema-`1.13.0` `serving-restore-rebuild@6`; backed-up projection, proof, mapping, FTS, Vectorize, lifecycle, and head rows are never restore truth. B2C-B3 retains canonical/evidence recovery, migration-away, protected data-access auditing, remote cold exercises, and measured 24-hour RPO/RTO. R2 native account Audit Logs are not treated as object read/write evidence. No local recovery seam advances the corresponding release gates.
