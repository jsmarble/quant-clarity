...
The contract-review candidate is locally implemented without changing any Worker, route, binding, migration, resource or blocker. Generated artifacts under `contracts/generated/provenance-v2/` draft strict canonical JSON, binary frame tags/lengths, the 32-path field/record-group corpus, raw-field mapping grammar, semantic-oracle obligations, table/root ownership and hash provenance, plus frame bytes/digests for each proposed leaf and collection domain. Closed TypeBox schemas cover `ProvenanceV2RegistrationPlan@1`, `ProvenanceV2AdapterReceipt@1`, the explicit `ProvenanceV2SuccessorManifest@1` preimage, raw mappings and pending aggregate limits. Focused review-candidate helpers reject representative unsafe hosts, paths, safe locators and raw-pointer extensions. Separate Node and WebCrypto tests reconstruct every vector without importing a shared encoder or hash helper.

These artifacts remain review evidence, not a normative registration contract. Machine-resolvable digest anchors, build-pinned external-artifact authority, canonical array ordering, nested/cross-table collection traversal, a full registration-plan/root vector, strict duplicate-detecting byte ingestion, immutable record-group instance identity, deterministic-derivation mappings, exact verifier matrices, complete hostile mutation vectors and independently recomputed successor child roots remain prerequisites. In particular, a caller-provided child root in the successor manifest is a claim for later oracle comparison and cannot confer authority.
