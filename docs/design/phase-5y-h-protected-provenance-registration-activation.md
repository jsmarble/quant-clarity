...
| Field | Value |
|---|---|
| Status | Proposed; review-candidate registration/root/field artifacts, typed root-binding plan, independent codec vectors, Phase 5Y-H1 frame composition, Phase 5Y-H2a complete preimages, Phase 5Y-H2b connected traversal evidence, Phase 5Y-H2c synthetic successor parity, Phase 5Y-H2d-a strict canonical-byte/selector evidence, Phase 5Y-H2d-b complete synthetic document/retained occurrence evidence, Phase 5Y-H2d-c document-derived refused cascade, Phase 5Y-H2e-a synthetic external-row resolution and Phase 5Y-H2e-b repository-path inventory locally implemented; persisted predecessor lookup, complete repository anchors/build manifest, semantic contract, accepted limits, migration and activation evidence pending |
...
The contract-review candidate is locally implemented without changing any Worker, route, binding, migration, resource or blocker. Generated artifacts under `contracts/generated/provenance-v2/` draft strict canonical JSON, binary frame tags/lengths, the 32-path field/record-group corpus, raw-field mapping grammar, semantic-oracle obligations, table/root ownership and hash provenance, plus frame bytes/digests for each proposed leaf and collection domain. The additive `root-binding-plan.v1.json` gives every registered digest and root-member count exactly one typed draft resolver; distinguishes leaf outputs, document values, retained bytes, approved D1 rows, repository artifacts whose build-manifest status remains pending, recomputed row references, collection references and record-frame outputs; fixes typed order/collation and heterogeneous table-family ordinals; binds all successor child count/root claims; and keeps complete resolver and migration-schema closure explicitly pending until migration 0010 exists. Closed TypeBox schemas cover `ProvenanceV2RegistrationPlan@1`, `ProvenanceV2AdapterReceipt@1`, the explicit `ProvenanceV2SuccessorManifest@1` preimage, the root-binding plan, raw mappings and pending aggregate limits. Focused review-candidate helpers reject representative unsafe hosts, paths, safe locators and raw-pointer extensions. Separate Node and WebCrypto tests reconstruct every existing codec vector without importing a shared encoder or hash helper. [Phase 5Y-H1](phase-5y-h1-independent-composite-root-evidence.md) composes all 33 isolated root-member leaf families through 38 family projections and the proposed higher-frame codec. [Phase 5Y-H2a](phase-5y-h2a-connected-registration-graph.md) adds the complete 371-row leaf-preimage inventory, [Phase 5Y-H2b](phase-5y-h2b-connected-traversal-vectors.md) independently recomputes all leaves, 386 typed projections, nine collection roots and synthetic authority/refused-frame linkage, [Phase 5Y-H2c](phase-5y-h2c-connected-successor-manifest-vectors.md) proves exact synthetic successor JCS and adapter-receipt parity, [Phase 5Y-H2d-a](phase-5y-h2d-a-canonical-registration-document-resolvers.md) proves strict duplicate-aware canonical bytes and independently executes all 45 checked document selector programs, [Phase 5Y-H2d-b](phase-5y-h2d-b-connected-registration-document.md) proves a complete schema-valid synthetic document, 31 actual safe-preimage occurrences, 46 scoped document counts and exact retained chunks, [Phase 5Y-H2d-c](phase-5y-h2d-c-connected-document-cascade.md) applies those commitments and independently regenerates the successor, leaves, collections, final document and refused cascade in Node and workerd, [Phase 5Y-H2e-a](phase-5y-h2e-a-external-row-resolvers.md) independently executes all seven external-row join/predicate/cardinality programs over three synthetic predecessor witnesses, and [Phase 5Y-H2e-b](phase-5y-h2e-b-repository-artifact-inventory.md) executes the eight available repository-path programs, inventories all ten bindings and verifies one present tracked artifact without inventing missing approvals or implementations. Persisted predecessor-row resolution, complete repository resolver/build-manifest closure and complete document-output/semantic parity remain pending.

These artifacts remain review evidence, not a normative registration contract. The root-binding plan makes the proposed digest/count targets and cross-table order machine-readable and locks the reviewed artifact as one canonical singleton. Phase 5Y-H1 supplies isolated family-projection vectors, Phase 5Y-H2a supplies the complete multi-row preimage inventory, Phase 5Y-H2b supplies connected synthetic leaf/order/traversal evidence plus non-authoritative higher-frame linkage, Phase 5Y-H2c supplies synthetic successor-manifest preimage parity, Phase 5Y-H2d-a supplies bounded strict byte admission plus checked document-selector execution, Phase 5Y-H2d-b supplies the complete synthetic document, occurrence/count inventory and retained-byte evidence, Phase 5Y-H2d-c supplies the additive document-to-graph digest overlay and refused cascade, Phase 5Y-H2e-a supplies synthetic external-row join/predicate/cardinality evidence, and Phase 5Y-H2e-b supplies the complete repository-binding path inventory plus one exact-byte tracked-file witness. Persisted external-row and complete repository resolvers, complete document-output and semantic parity, D1 enumeration and row-lookup closure, the independent semantic oracle, migration-0010 schema parity, build-manifest verification, immutable record-group instance identity, deterministic-derivation mappings, exact verifier matrices and complete hostile mutation vectors remain prerequisites. In particular, a caller-provided child root, successor hash or predecessor digest remains only a claim for later oracle comparison and cannot confer authority.
