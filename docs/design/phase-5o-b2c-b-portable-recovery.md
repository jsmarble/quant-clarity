...
The maximum-object proof deliberately uses a test-only hostile-but-canonical packer because the production writer's deterministic 2 MiB target does not naturally create 63 source chunks within the 24 MiB aggregate cap. This tests the verifier's independent admission boundary without changing the production format, writer, or reviewed limits. Remote private-bucket, lock, worst-accepted-size Worker, and operational recovery evidence remain pending; local B1 evidence does not claim those release outcomes.
