...
The next slice must implement strict duplicate-detecting canonical registration-document byte ingestion and the typed document/retained-byte selectors, then resolve approved external and repository anchors against a reviewed build manifest. Semantic-oracle closure, migration/D1 guards and accepted-scale evidence remain later gates.
