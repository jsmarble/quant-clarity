...
## Open contract/storage mismatch

Review identified a pre-existing mismatch outside this reader: `ProviderSchema` admits C0 NUL in a known display name, while SQLite `length(TEXT)` stops at the first NUL and migration 0007's `display_name` length check can reject that otherwise contract-valid value. This slice does not reinterpret the approved normalization or canonical contract. A dedicated ADR/control must decide whether to prohibit NUL at the contract boundary or replace the storage check with byte/scalar-safe validation, then add migration and cross-runtime evidence. Search requirements remain `Planned` while this and the complete acceptance gates are unresolved.
