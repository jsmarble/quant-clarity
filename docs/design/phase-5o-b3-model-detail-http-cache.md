...
Status: implementation in progress; B3-A admission and pre-open audit plus the closed, unrouted B3-B Model-detail HTTP composition, manual stable-ID cache boundary, generated Model-detail OpenAPI contract, and Phase 5P-D exact local protected runtime assembly implemented locally; proposed ADR 0045 would require product-owner-approved schema-1.14 lifecycle-v6 three-artifact recovery admission before routing; protected audit invocation/evidence, recovery admission, preview/production origin and transport policy, public routing, and release evidence pending
...
Both planners remain pure and are not imported by the live Worker handler. A closed Worker-native renderer now copies accepted plan bytes into real `Response` objects, rejects hostile or crossed header/status plans to one generic private `503`, preserves bodyless HEAD/`304`/`308`, supplies the fixed post-limiter preflight response, and applies only an explicit local/test, preview HTTPS, or production-custom-hostname HTTPS transport policy. Unit and pinned-workerd Response-object tests cover representation length, byte detachment, conditional/redirect/error body rules, exact CORS/security headers, and the HSTS matrix; actual routed HTTP-wire conformance remains pending with route integration. The local API Wrangler configuration explicitly disables pre-invocation Workers Caching, and the zero-visitor-data configuration gate rejects omission, enablement, or extension of that setting. [ADR 0048](../decisions/0048-unrouted-model-detail-protected-runtime.md) and [Phase 5P-D](phase-5p-d-unrouted-model-detail-runtime.md) now bind the internal composition to the exact local `DEPLOYMENT_ENV=local`, `PUBLIC_API_ORIGIN=https://api.example.test`, and `API_TRANSPORT_POLICY=local_test` tuple plus actual local Worker runtime capabilities. The live export remains unchanged, and handler/runtime tests continue to prove Model stable-ID and slug paths return `404` without Model-detail RPC or Cache API effects. Preview/production origin and transport selection, public routing, complete routed HTTP-wire evidence, and deployment remain pending.
