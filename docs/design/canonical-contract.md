...
- Private provenance-v2 operational IDs use `pvi_` for a physical installation, `vpa_` for an authority plan, `sep_` for an approved source endpoint, `pvb_` for a Provider bundle, `pvp_` for an acquisition permit, and `prs_` for an admitted response. They are not public resource identities and are intentionally excluded from the public `IdPrefixSchema` and `ResourceIdSchema`; no public contract or route may accept them as a resource ID.
