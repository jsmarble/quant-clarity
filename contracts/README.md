...
- an authoritative required `SearchCollection.meta.semantic_degraded` state that remains expressible for empty exact/structured fallback, with an identical required `/v1` result mirror and no implicit default (`API-004`, `API-010`, `API-016`, `NFR-006`);
- semantic validation for adapter manifests, terminal batches, publication closure, and head activation; and
- non-public provenance-v2 registration-plan, adapter-receipt, raw-field-mapping, field-corpus, canonical-byte, root-registry, semantic-policy, and independent golden-vector review candidates for proposed ADR 0067. These target migration 0010, admit only `benchmark_pending`, always refuse authority inspection, and confer no source, registration, public, remote, or deployment authority.

Remaining contract work includes the authenticated cursor and search-document/index schemas, validated human-readable examples for every public resource, provenance-v2 contract-review closure, and accepted-scale workerd evidence fixing aggregate limits before any registration plan can pass authority validation. The Model-detail route now has a validated canonical success example and exact ADR 0044 response matrix; this does not claim routed or deployed behavior. ADR 0024 resolves the empty-search degradation representation only; provider-only semantic applicability and the merged public search cursor remain pending. Active path-only details are cacheable through a publication-scoped internal cache key; collections and every request with a query string are `private, no-store`. ADR 0013 defines `X-QuantClarity-Publication` as the public publication pin, and ADR 0016 defines the bounded local cursor, service-envelope, conditional-request, and cache-origin decisions without claiming deployed runtime behavior.
