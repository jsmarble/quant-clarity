...
const LOCAL_CURSOR_KEY = new TextEncoder().encode(
  "quantclarity-local-test-only-exact-variant-search-cursor-key-v1",
);
...
type LocalTestEnvironment = "local" | "test";

const localTestEnvironment = (value: unknown): value is LocalTestEnvironment =>
  value === "local" || value === "test";

export type AdmittedExactVariantSearchCapabilities = Readonly<{
...
/** Executes only a purpose-separated request that crossed signed admission. */
export const handleAdmittedExactVariantSearch = async (
  request: NormalizedRequest,
  capabilities: AdmittedExactVariantSearchCapabilities,
): Promise<Response> => {
  try {
    if (
      !localTestEnvironment(capabilities.environment) ||
    ...
    const outcome = await readExactVariantCardSearchFromQueryV1({
    ...
    return new Response(new Uint8Array(encoded.representationBytes), {
      ...
      headers: {
        ...
        "Content-Type": "application/json; charset=utf-8",
        Vary: "X-QuantClarity-Publication",
        "X-QuantClarity-Publication": encoded.collection.meta.publication_id,
      },
    });
  } catch {
    return error(
...
type RuntimeBindings = Readonly<{ CATALOG_QUERY: Service }>;

/** Captures the query capability only after signature and cursor admission. */
export const handleAdmittedExactVariantSearchRuntime = (
  request: NormalizedRequest,
  bindings: RuntimeBindings,
  environment: LocalTestEnvironment,
  nowMs: number,
): Promise<Response> => {
  if (!localTestEnvironment(environment))
  ...
  return authenticateCursor().then((authenticated) => {
    ...
    const queryRpc: ExactVariantCardSearchCatalogQueryRpcV1 = {
      resolvePublicationV2: (input) => invoke("resolvePublicationV2", input),
      readExactVariantCardSearchV1: (input) =>
        invoke("readExactVariantCardSearchV1", input),
    };
    return handleAdmittedExactVariantSearch(request, {
      environment,
      nowMs: () => nowMs,
